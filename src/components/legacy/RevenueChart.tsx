  ...
  useEffect(() => {
    ...
    void chartModule().then((Chart) => {
      ...
      chartRef.current = new Chart(canvas, {
        type: "line",
        data: {
          labels: data.map((row) => new Date(row.date).toLocaleDateString(locale, { day: "2-digit", month: "short" })),
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-emerald-400">{new Intl.NumberFormat(locale, { style: "currency", currency: "EUR" }).format(total)}</span>
            </div>
          </div>
        </div>
        <div className="flex rounded-lg border border-white/5 bg-white/5 p-1">
          {periods.map((period) => (
            <button key={period} type="button" onClick={() => setDays(period)} data-days={period} data-active={days === period ? "true" : undefined} className="period-btn rounded-md px-3 py-1.5 text-xs font-medium text-slate-400 transition-all hover:text-white data-[active=true]:bg-white/10 data-[active=true]:text-white data-[active=true]:shadow-sm">
              {t(`chart.days${period}`)}
            </button>
          ))}
        </div>
