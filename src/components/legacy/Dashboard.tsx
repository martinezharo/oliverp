...
import SoloVatOverview from "@/components/plugins/SoloVatOverview";

const t = (key: string) => ui.en[key] ?? key;

export default function Dashboard() {
  const { projectId, pluginEffects, openModal } = useErpContext();
  ...
  return (
    <>
      ...
      {rows === undefined ? <StatsSkeleton /> : pluginEffects.includes("dashboard.solo_iva") ? <SoloVatOverview transactions={rows} /> : <DashboardStats transactions={rows} />}
      {!pluginEffects.includes("dashboard.solo_iva") && <RevenueChart projectId={projectId} />}
    </>
  );
}
