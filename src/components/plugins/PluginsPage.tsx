...
import { installArgs, type PluginEffect, type PluginInstallation, type ResolvedPlugin } from "@/lib/plugins";

const effectCopy: Record<PluginEffect, { label: string; detail: string }> = {
  "dashboard.solo_iva": {
    label: "Only VAT transactions",
    detail: "Extends OlivERP's normal transaction workflow with the Solo IVA behavior.",
  },
};
    ...
    <div className="space-y-8">
      <header className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">Add your own private extensions and activate their effects inside OlivERP. There is no public catalog.</p>
        </div>
      ...
      {!projectId ? <EmptyState title="Select a project" detail="Private plugins are added and activated independently for each project." /> : (
        <>
          <section className="overflow-hidden rounded-3xl border border-white/[0.07] bg-[#14151a]/55">
            <div className="border-b border-white/5 px-6 py-5"><h2 className="text-lg font-semibold text-white">Add a private plugin</h2><p className="mt-1 text-sm text-slate-500">Paste a GitHub repository you own. OlivERP reads its declarative manifest; it does not open or execute an external application.</p></div>
          ...
          <section>
            <div className="mb-4 flex items-end justify-between"><div><h2 className="text-lg font-semibold text-white">Your plugins</h2><p className="mt-1 text-sm text-slate-500">Active effects are applied directly to the selected project&apos;s normal screens.</p></div>{rows && <span className="rounded-full border border-white/5 bg-white/[0.025] px-3 py-1 text-xs text-slate-500">{rows.length} added</span>}</div>
            {loading ? <div className="grid gap-4 md:grid-cols-2"><PluginSkeleton /><PluginSkeleton /></div> : !rows?.length ? <EmptyState title="No private plugins added" detail="Paste one of your GitHub repositories above to add the first extension." /> : <div className="grid gap-4 md:grid-cols-2">{rows.map((plugin) => <article key={plugin.pluginId} className={`rounded-3xl border p-6 transition-colors ${plugin.enabled ? "border-primary-500/20 bg-primary-500/[0.035]" : "border-white/[0.07] bg-[#14151a]/55"}`}><div className="flex items-start gap-4"><PluginMark enabled={plugin.enabled} /><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-4"><div><h3 className="font-semibold text-white">{plugin.name}</h3><p className="mt-1 text-xs text-slate-600">v{plugin.version} · private repository</p></div><button type="button" role="switch" aria-checked={plugin.enabled} aria-label={`${plugin.enabled ? "Deactivate" : "Activate"} ${plugin.name}`} disabled={changing === plugin.pluginId} onClick={() => void toggle(plugin)} className={`relative h-7 w-12 shrink-0 rounded-full border transition-colors disabled:opacity-50 ${plugin.enabled ? "border-primary-400/30 bg-primary-500" : "border-white/10 bg-white/5"}`}><span className={`absolute top-1 h-[18px] w-[18px] rounded-full bg-white shadow transition-transform ${plugin.enabled ? "left-1 translate-x-5" : "left-1"}`} /></button></div><p className="mt-3 text-sm leading-6 text-slate-400">{plugin.description}</p><div className="mt-4 space-y-2">{plugin.effects.map((item) => <div key={item} className="flex items-start gap-2 text-xs text-slate-500"><span className={`mt-1 h-1.5 w-1.5 shrink-0 rounded-full ${plugin.enabled ? "bg-primary-400" : "bg-slate-700"}`} /><span>{effectCopy[item]?.label ?? item}</span></div>)}</div></div></div><div className="mt-5 flex items-center gap-3 border-t border-white/5 pt-4"><a href={plugin.repositoryUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm text-slate-500 transition-colors hover:text-white"><LockIcon className="h-3.5 w-3.5" /> Repository</a><span className={`text-xs font-semibold ${plugin.enabled ? "text-emerald-400" : "text-slate-600"}`}>{plugin.enabled ? "Active in OlivERP" : "Inactive"}</span><button type="button" onClick={() => setRemoving(plugin)} className="ml-auto rounded-lg px-2 py-1.5 text-xs text-slate-600 transition-colors hover:bg-red-500/10 hover:text-red-300">Remove</button></div></article>)}</div>}
          </section>
        </>
      )}
