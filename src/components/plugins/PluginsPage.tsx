...
const slotCopy: Record<PluginSlot, { label: string; detail: string }> = {
  "dashboard.summary": {
    label: "Dashboard financial summary",
    detail: "Renders a validated native summary in place of the standard dashboard cards.",
  },
};

const permissionCopy: Record<PluginPermission, { label: string; detail: string }> = {
  "finances:read": {
    label: "Read project finance summaries",
    detail: "Sends daily financial totals for this project to the plugin runtime when the dashboard loads.",
  },
};
    ...
    <div className="space-y-8">
      <header className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">Connect your own private repositories. Their validated output appears inside OlivERP and persists for the selected project. There is no public catalog.</p>
        </div>
      ...
      {!projectId ? <EmptyState title="Select a project" detail="Private plugins are added and activated independently for each project." /> : (
        <>
          <section className="overflow-hidden rounded-3xl border border-white/[0.07] bg-[#14151a]/55">
            <div className="border-b border-white/5 px-6 py-5"><h2 className="text-lg font-semibold text-white">Add a private plugin</h2><p className="mt-1 text-sm text-slate-500">Paste a private GitHub repository you own. OlivERP reviews its runtime, requested data permissions, and native UI slot before installation.</p></div>
          ...
          <section>
            <div className="mb-4 flex items-end justify-between"><div><h2 className="text-lg font-semibold text-white">Your plugins</h2><p className="mt-1 text-sm text-slate-500">Active plugins render directly in the selected project&apos;s normal screens.</p></div>{rows && <span className="rounded-full border border-white/5 bg-white/[0.025] px-3 py-1 text-xs text-slate-500">{rows.length} added</span>}</div>
